...
int main (void){
  ...
  while(true){
    ...
    while(receiveASKByte() != ACK);         //Wait for ACK (Blocking)

    bool more = true;
    while(more){
      transmitASKByte(NEXT_COMMAND);        //NEXT_COMMAND
...
void initialize() {
  ...
  pinMode(SERIAL_ENABLE, OUTPUT);      //*********************FIX ME (no longer needed on Mk.II)**********************
  digitalWrite(SERIAL_ENABLE, HIGH);   //*********************FIX ME (no longer needed on Mk.II)**********************
  enableSerial();                      //*********************FIX ME Should be disabled by defualt**********************
...
bool processASKCommands(byte command){
    if(command == NULL || command == SLEEP){
        return false;
